from pyspark.sql.functions import col, hour, when, round, count, avg, percentile_approx, sum, desc, to_date, dense_rank
...
def add_speed_mph(df:DataFrame) -> DataFrame:
    ...
    df = df.withColumn(
        "is_suspicious_speed",
        when((col("speed_mph") > 80), True).otherwise(False)
    )
    return df
...
def cumulative_trips_by_day(df:DataFrame) -> DataFrame:
    ...
    window_spec = Window.orderBy("pickup_date").rowsBetween(Window.unboundedPreceding, 0)

    df = df.withColumn(
        "cumulative_trip_count",
        sum("daily_trip_count").over(window_spec)
    ).orderBy("pickup_date")
    return df

# Task 5.2 — Rank hours within each day
def rank_hours_by_day(df:DataFrame) -> DataFrame:
    df = df.groupBy("pickup_date","pickup_hour").agg(
        count("*").alias("trip_count")
    )
    window_part = Window.partitionBy("pickup_date").orderBy(desc("trip_count"))
    df = df.withColumn(
        "daily_hour_rank",
        dense_rank().over(window_part)
    )
    return df
